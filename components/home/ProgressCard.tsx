...
export function ProgressCard() {
  ...
  const accuracy = percentage(totalCorrect, totalQuestions);

  const stats = [
    { label: "学習回数", value: `${history.length}`, unit: "回", icon: "✅" },
    { label: "平均正答率", value: `${accuracy}`, unit: "%", icon: "🎯" },
  ...
  return (
    <section className="rounded-[2rem] border border-slate-100 bg-white p-5 shadow-card">
      <div className="flex items-center justify-between">
        ...
        <div className="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-black text-emerald-600">Good pace</div>
      </div>
      ...
      <div className="mt-4">
        ...
        <div className="mt-2 h-2.5 overflow-hidden rounded-full bg-slate-100">
          <div className="h-full rounded-full bg-gradient-to-r from-brand-600 to-orange-400" style={{ width: `${Math.min(accuracy || 12, 100)}%` }} />
        </div>
      </div>
    </section>
  );
