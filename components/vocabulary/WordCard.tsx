  ...
  return (
    <article className="rounded-[1.7rem] border border-slate-100 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xl font-black text-slate-950">{word.word}</p>
          <p className="mt-1 text-sm font-black text-brand-700">{word.meaning}</p>
        </div>
        <span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black text-slate-500">{word.category}</span>
      </div>
